            ...
            public static void Main(string[] args)
            {
                ...
                try
                {
                    ...
                    using (var luaEngine = new LuaEngine(config, maps, mbDungeons, rcDungeons, itemPickupFilter, simulation))
                    {
                        ...
                        // Improved Lua script loading and bot loop with error handling
                        string logFile = Path.Combine(config.MainDir, "error.log");
                        void LogError(string message)
                        {
                            Console.WriteLine(message);
                            try { File.AppendAllText(logFile, message + Environment.NewLine); } catch { }
                        }
                        ...
                        foreach (string luaFile in coreLuaFiles)
                        {
                            ...
                            catch (Exception ex) { LogError($"Error loading core Lua script {Path.GetFileName(luaFile)}: {ex.Message}"); }
                        }
                        ...
                        {
                            try { lua.DoFile(luaFile); }
                            catch (Exception ex) { LogError($"Error loading Lua script {Path.GetFileName(luaFile)}: {ex.Message}"); }
                        }
                        // Simulate bot operation (OnTick loop)
                        Console.WriteLine("\nSimulating bot operation... Press Ctrl+C to stop.");
                        try { lua.DoString("if OnLoad then OnLoad() end"); } catch (Exception ex) { LogError($"Error in OnLoad: {ex.Message}"); }
                        try { lua.DoString("if OnStartBot then OnStartBot() end"); } catch (Exception ex) { LogError($"Error in OnStartBot: {ex.Message}"); }
                        bool running = true;
                        Console.CancelKeyPress += (s, e) => { running = false; e.Cancel = true; };
                        while (running)
                        {
                            try { lua.DoString("if OnTick then OnTick() end"); }
                            catch (Exception ex) { LogError($"Error during Lua OnTick: {ex.Message}"); running = false; }
                            System.Threading.Thread.Sleep(100);
                        }
                        Console.WriteLine("Simulation stopped.");
                        try { lua.DoString("if OnStopBot then OnStopBot() end"); } catch (Exception ex) { LogError($"Error in OnStopBot: {ex.Message}"); }
                    }
                }
                catch (Exception ex)
                {
